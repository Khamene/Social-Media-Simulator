                ...
                if (!typeField.getText().equals("")) {
                    if (kp.getCode().equals(KeyCode.ENTER)) {
                        if (myChat.getGroup()) {
                            ...
                            catch (GroupDoesNotExistException | NoUserLoggedInException | UserDoesNotExistException | MessageContentNullException | UserNotMemberException | UnauthorisedEditException e) {
                                e.printStackTrace();
                            }
                            catch (UserWasBannedException e) {
                                typeField.setText("You were banned by the admin");
        ...
        if (chatFilter == 0) {
            for (Chat chat : chats) {
                ...
                if (chat.getPhotoPath() != null) {
                    Image myImage = new Image(String.valueOf(this.getClass().getResource(chat.getPhotoPath())));
