...
public class SQLManager {
    ...
    public static int blockUser(String blockerID, String blockedID) {
        String query = String.format("SELECT USERNAME FROM USERS WHERE USERNAME = \"%s\"", blockedID);
    ...
    public static int sendFollowRequest(String username, String otherUsername) {
        String query = String.format("SELECT USERNAME FROM USERS WHERE USERNAME = \"%s\"", otherUsername);

        try {
            ResultSet resultSet = statement.executeQuery(query);

            if (resultSet.next()) {
                query = String.format("SELECT * FROM FOLLOWREQUESTS WHERE FROMID = \"%s\" && TOID = \"%s\"", username, otherUsername);
                ...
                if (resultSet.next())
                    ...
                    query = String.format("SELECT * FROM RELATIONS WHERE FOLLOWERID = \"%s\" && FOLLOWEDID = \"%s\"", username, otherUsername);
                    ...
                    if (resultSet.next())
                        return -2;
                         ...
                         query = String.format("INSERT INTO FOLLOWREQUESTS VALUES (\"%s\", \"%s\")", username, otherUsername);
    ...
    public static int acceptFollowRequest(String username, String otherUsername) {
        String query = String.format("SELECT USERNAME FROM USERS WHERE USERNAME = \"%s\"", otherUsername);
        ...
        try {
            ...
            if (resultSet.next()) {
                query = String.format("SELECT FOLLOWEDID FROM RELATIONS WHERE FOLLOWEDID = \"%s\" && FOLLOWERID = \"%s\"", username, otherUsername);
                ...
                if (resultSet.next())
                    ...
                    query = String.format("SELECT FROMID FORM FOLOWREQUESTS WHERE FROMID = \"%s\" && TOID = \"%s\"", otherUsername, username);

                    resultSet = statement.executeQuery(query);

                    if (resultSet.next()) {
                        query = String.format("DELETE FORM FOLOWREQUESTS WHERE FROMID = \"%s\" && TOID = \"%s\"", otherUsername, username);
                        statement.execute(query);
                        query = String.format("INSERT INTO RELATIONS VALUES (\"%s\", \"%s\")", otherUsername, username);
                        statement.execute(query);
