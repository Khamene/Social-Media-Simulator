...
//            System.out.println(e.getMessage());
    ...
    public static ArrayList<Comment> getComments2(String postID) throws SQLException {
        String query = String.format("SELECT USERS.USERNAME, COMMENTS.COMMENT FROM COMMENTS" +
                " INNER JOIN USERS ON USERS.ID = COMMENTS.USERID WHERE COMMENTS.POSTID = \"%s\"" +
                " ORDER BY DATECOMMENTED", postID);
        ...
        while (resultSet.next()) {
            ...
            comments.add(new Comment(postID, username, content));
        }
    ...
    public static int unlikePost(String userID, String postID) throws SQLException {
        ...
        if (resultSet.next()) {
            ...
            if (found) {
                ...
                resultSet.next();
                if (resultSet.getString(1).equalsIgnoreCase("B")) {
                    query = String.format("UPDATE ADVERTISEMENTS SET LIKED = 0 WHERE POSTID = \"%s\"", postID);
                    statement.execute(query);
                }
...
//            System.out.println(e.getMessage());
    ...
    public static void publishPost(String userID, String postID, String content, String filePath) throws SQLException{
        String query = String.format("INSERT INTO POSTS VALUES (\"%s\", \"%s\", 0, 0, \"%s\", \"%s\", \"%s\")"
                , postID, userID, content, filePath, LocalDateTime.now().toString());
    ...
    public static boolean getLiked(String userID, String postID) throws SQLException {
